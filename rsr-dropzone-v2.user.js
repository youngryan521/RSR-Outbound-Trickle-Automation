...
(function () {
  ...
  function runMove() {
    ...
    const tryScanDestId = async destId => {
      ...
      console.log('[DZ+] infodisplay:', infoText() || '(empty)', '| sdMsg:', sdMsg() || '(empty)');
      if (/already scanned to container/i.test(info))                                                  { console.log('[DZ+] Already in cart -- skipping counter'); return 'already_done'; }
    ...
    async function loop() {
      ...
      while (true) {
        try {
          ...
          else if (result === 'already_done') { s2.action = 'already_done'; save(s2); console.log('[DZ+] ALREADY DONE:', s.sp00); }
