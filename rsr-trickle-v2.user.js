...
(function () {
  ...
  function runRodeo() {
    ...
    const addToSkipList = (s, id, reason) => {
      ...
      console.log('[RSR+][Rodeo] Permanent skip (' + reason + '):', id);
    };
    ...
    (function shiftStartCheck() {
      ...
      if (elapsed > SHIFT_GAP_MS) {
        ...
        console.log('[RSR+][Rodeo] New shift detected (' + Math.round(elapsed / 3600000) + 'h idle) -- clearing skipList (' + prev + ' entries)');
        s0.skipList = [];
      }
    ...
    async function loop() {
      ...
      while (true) {
        try {
          ...
          // Handle outcome reported by the Trickle side
          if (s.action === 'done') {
            s[okKey[s.cpt]]++;
            addRecent(s.rawId);
            ...
            if (s.rawId) {
              ...
              if (errorCount[s.rawId] >= 3) {
                ...
                console.log('[RSR+][Rodeo] Cooldown:', s.rawId, '(error #' + errorCount[s.rawId] + ')');
              }
            }
            addRecent(s.rawId);
            s.action = 'idle'; save(s);
          ...
          if (!pick) {
            if (anyAvailable) {
              ...
              console.log('[RSR+][Rodeo] All recently processed -- resetting');
            }
            continue;
          }
